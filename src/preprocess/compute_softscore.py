...
if __name__ == '__main__':
    debug = False
    if debug:
        debug_answer_file = 'data/test_annotations.json'
        debug_answers = json.load(open(debug_answer_file))['annotations']
        answers = debug_answers
        occurence = filter_answers(answers, 0)
        ans2label = create_ans2label(occurence, 'only_debug')
        compute_target(debug_answers, ans2label, 'debug')
    else:
        train_answer_file = 'data/mscoco_train2014_annotations.json'
        train_answers = json.load(open(train_answer_file))['annotations']
        val_answer_file = 'data/mscoco_val2014_annotations.json'
