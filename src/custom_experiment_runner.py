from student_code.custom_net import CustomNet
from student_code.experiment_runner_base import ExperimentRunnerBase
from student_code.vqa_dataset import VqaDataset
import torch.nn as nn
import torch

class CustomNetExperimentRunner(ExperimentRunnerBase):
    """
    ...
    def __init__(self, train_image_dir, train_question_path, train_annotation_path,
        ...
        if not args.debug:
            ...
            model = CustomNet(num_ans_candidates=2185, ntoken=train_dataset.dictionary.ntoken)
        else:
            train_dataset = VqaDataset(image_dir=train_image_dir,
                                       question_json_file_path=train_question_path,
                                       annotation_json_file_path=train_annotation_path,
            ...
            model = CustomNet(num_ans_candidates=2, ntoken=train_dataset.dictionary.ntoken)
        super().__init__(train_dataset, val_dataset, model, batch_size, num_epochs,
                         num_data_loader_workers, args.use_cuda, 'custom')
